<?php
	...
	// widgets novos
	include( TEMPLATEPATH . '/widget/widget-referencias.php' );
	include( TEMPLATEPATH . '/widget/widget-ferramentas.php' );
	include( TEMPLATEPATH . '/widget/widget-padroes.php' );
	include( TEMPLATEPATH . '/widget/widget-perspectivas.php' );
	include( TEMPLATEPATH . '/widget/widget_categorias_filhas.php' );
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">	
			...
			<div class="comentario">
				...
				<?php if ($comment->comment_approved == '0') : ?>
					<p><?php _e('Your comment is waiting moderation.') ?></p>
				<?php else: ?>
					<?php comment_text() ?>
				<?php endif; ?>
				<?php if( comment_reply_link(array_merge( $args, array('reply_text' => __('Reply', 'evoeTheme'), 'depth' => $depth, 'max_depth' => $args['max_depth']))) != '' ) { comment_reply_link(array_merge( $args, array('reply_text' => __('Reply', 'evoeTheme'), 'depth' => $depth, 'max_depth' => $args['max_depth']))); } else { echo '<div class="reply-link"></div>'; } ?>
			</div>
			<div class="clear"></div>
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
			<div class="comment-reply">
				...
				<?php else : ?>
					<?php comment_text() ?>
				<?php endif; ?>
				<?php comment_reply_link(array_merge( $args, array('reply_text' => __('Reply', 'evoeTheme'), 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
				<div class="autorResposta">
					<p class="nomeData"><?php print get_comment_author_link() . ' <span>' . get_comment_date( __('m, jS', 'evoeTheme') ) . '</span>'; ?></p>
