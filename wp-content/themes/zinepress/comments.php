<?php // Do not delete these lines
	...
	if (!empty($post->post_password)) { // if there's a password
		if ($_COOKIE['wp-postpass_' . COOKIEHASH] != $post->post_password) {  // and it doesn't match the cookie
			...
			<p class="nocomments">Este artigo não permite comentários</p>
...
<?php if ($comments) : ?>
	<h3 id="comments"><?php comments_number('Deixe seu comentário', 'Um comentário', '% comentários' );?> para &#8220;<?php the_title(); ?>&#8221;</h3>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li <?php echo $oddcomment; ?>id="comment-<?php comment_ID() ?>">
			...
			<div class="commentmetadata"><a href="#comment-<?php comment_ID() ?>" title=""><!-- uncomment to show time <?php comment_time() ?> on --><?php comment_date('F jS, Y') ?> </a> <?php edit_comment_link('editar','&nbsp;&nbsp;',''); ?></div></div>

			<div class="commenttext"><?php if ($comment->comment_approved == '0') : ?>
			<em>Seu recado está aguardando moderação.</em>
...
<h3 id="respond">Adicionar comentário</h3>
