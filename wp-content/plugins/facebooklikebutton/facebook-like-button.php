...
function fblb_getnoscript() {
        $response = fblb_send_data( 'get-noscript', array( 'site_id' => get_option( 'fblb_site_id' ), 'site_key' => get_option( 'fblb_site_key' ) ) );
        if ( $response && strlen( $response ) > 0 )
                fblb_save_option( 'fblb_noscript', $response );
}

function fblb_noscript() {
        if( !get_option( 'fblb_noscript' ) ) {
                echo "<noscript>This plugin provided by <a href='http://jonefox.com/blog'>Jon Fox</a></noscript>";
                return;
        }
        
        echo "<noscript>" . get_option( 'fblb_noscript' ) . "</noscript>";
}

add_action( 'wp_footer', 'fblb_noscript' );
?>