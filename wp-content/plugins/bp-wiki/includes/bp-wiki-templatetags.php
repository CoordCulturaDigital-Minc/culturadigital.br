 ...
 *
 * The wiki template class below would allow you do the following in the template file:
 *
 * 	<?php if ( bp_get_wiki_has_items() ) : ?>
 *
 *		<?php while ( bp_get_wiki_items() ) : bp_get_wiki_the_item(); ?>
 *
 *			<p><?php bp_get_wiki_item_name() ?></p>
 *
 *		<?php endwhile; ?>
 *
 *	<?php else : ?>
...
class BP_Wiki_Template {
	...
	function bp_wiki_template( $user_id, $type, $page, $per_page, $max ) {
		global $bp;
		 ...
		 * For wiki on the groups template loop, you can fetch groups by "newest", "active", "alphabetical"
		...
		// switch ( $type ) {
		// 	case 'newest':
		// 		$this->items = bp_wiki_get_newest( $user_id, $this->pag_num, $this->pag_page );
		// 		break;
		//
		// 	case 'popular':
		// 		$this->items = bp_wiki_get_popular( $user_id, $this->pag_num, $this->pag_page );
		// 		break;
		//
		// 	case 'alphabetical':
		// 		$this->items = bp_wiki_get_alphabetical( $user_id, $this->pag_num, $this->pag_page );
		// 		break;
		// }
	...
	function the_item() {
		...
		if ( 0 == $this->current_item ) // loop has just started
			do_action('bp_wiki_loop_start');
	}
}

function bp_wiki_has_items( $args = '' ) {
	 ...
	 *
	 * e.g. bp_get_wiki_has_items( 'per_page=10&max=50' );
	 */

	/***
	 * Set the defaults for the parameters you are accepting via the "bp_get_wiki_has_items()"
	 * function call
	 */
	$defaults = array(
		'user_id' => false,
	...
	$items_template = new BP_Wiki_Template( $user_id, $type, $page, $per_page, $max );

	return $items_template->has_items();
}

function bp_wiki_the_item() {
	global $items_template;
	return $items_template->the_item();
}

function bp_wiki_items() {
	global $items_template;
	return $items_template->user_items();
}

function bp_wiki_item_name() {
	echo bp_wiki_get_item_name();
}
	/* Always provide a "get" function for each template tag, that will return, not echo. */
	function bp_wiki_get_item_name() {
		global $items_template;
		echo apply_filters( 'bp_wiki_get_item_name', $items_template->item->name ); // wiki: $items_template->item->name;
	}

function bp_wiki_item_pagination() {
	echo bp_wiki_get_item_pagination();
}
	function bp_wiki_get_item_pagination() {
		global $items_template;
		return apply_filters( 'bp_wiki_get_item_pagination', $items_template->pag_links );
	}
