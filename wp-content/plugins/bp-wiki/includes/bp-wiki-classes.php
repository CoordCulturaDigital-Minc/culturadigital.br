...
class BP_Wiki_TableName {
	 ...
	 */
	function bp_wiki_tablename( $id = null ) {
		global $wpdb, $bp;
	...
	function populate() {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp->wiki->table_name} WHERE id = %d", $this->id ) ) ) {
	...
	function save() {
		 ...
		 * 
		 * 1. Blanket filtering of values by plugins (for wiki if a plugin wanted to force a specific 
		 *	  value for all saves)
		 * 
		 ...
		 * 'bp-wiki-filters.php'
		 *
		 *   add_filter( 'wiki_data_fieldname1_before_save', 'wp_filter_kses' );
		 */	
		
		$this->fieldname1 = apply_filters( 'bp_wiki_data_fieldname1_before_save', $this->fieldname1, $this->id );
		$this->fieldname2 = apply_filters( 'bp_wiki_data_fieldname2_before_save', $this->fieldname2, $this->id );
		
		/* Call a before save action here */
		do_action( 'bp_wiki_data_before_save', $this );
						
		if ( $this->id ) {
			// Update
			$result = $wpdb->query( $wpdb->prepare( 
					"UPDATE {$bp->wiki->table_name} SET 
						field_1 = %d,
						field_2 = %d,
						field_3 = %d
					WHERE id = %d",
			...
			// Save
			$result = $wpdb->query( $wpdb->prepare( 
					"INSERT INTO {$bp->wiki->table_name} ( 
						field_1,
						field_2,
						field_3 
					) VALUES ( 
		...
		do_action( 'bp_wiki_data_after_save', $this ); 
	...
	function delete() {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$bp->wiki->table_name} WHERE id = %d", $this->id ) );
	}
	 ...
	 *
	 * Look at bp-core-classes.php for wikis of mass delete.
	 */
