...
function bp_wiki_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	if ( 'no' == get_usermeta( (int)$to_user_id, 'notification_wiki_new_high_five' ) )
		return false;
	...
	$sender_highfive_link = site_url( BP_MEMBERS_SLUG . '/' . $sender_ud->user_login . '/' . $bp->wiki->slug . '/screen-one' );
	...
	$subject = '[' . get_blog_option( 1, 'blogname' ) . '] ' . sprintf( __( '%s high-fived you!', 'bp-wiki' ), stripslashes($sender_name) );
...
---------------------
', 'bp-wiki' ), $sender_name, $sender_name, $sender_profile_link, $sender_name, $sender_highfive_link );

	$message .= sprintf( __( 'To disable these notifications please log in and go to: %s', 'bp-wiki' ), $reciever_settings_link );

	// Send it!
	wp_mail( $to, $subject, $message );
}
add_action( 'bp_wiki_send_high_five', 'bp_wiki_send_high_five_notification', 10, 2 );
