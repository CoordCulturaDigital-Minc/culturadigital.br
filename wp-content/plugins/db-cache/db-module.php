...
<p>If you don\'t know how to setup a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, DB_USER));
	...
	function query($query) {
		...
		if ( ! $this->ready )
			...
			// Caching
			$plugin_dir = WP_CONTENT_DIR."/plugins/db-cache";
			require_once $plugin_dir."/db-functions.php";
			$config = unserialize(@file_get_contents(WP_CONTENT_DIR."/db-config.ini"));
				if (!isset($config['fiter']) || strlen($config['fiter']) < 3) $config['fiter'] = 'insert|delete';
			
			$dbc = new pcache();
				$dbc->lifetime = $config['timeout'];
				...
				// Clean unused
				$dbcheck = date('G')/4;
				if ($dbcheck == intval($dbcheck) && $config['lastclean'] < time() - 3600) {
					$dbc->clean();
			...
			if (preg_match("/\\s*(".$config['filter'].")/si",$query)) $dbc_cachable = false;
...
Debug part
*/
if ($config['debug'] && $dbc_cachable) echo "\n<!-- cache: $query -->\n\n";
elseif ($config['debug'] && !$dbc_cachable) echo "\n<!-- mysql: $query -->\n\n";
		
			//elseif ( preg_match("/^\\s*(_options|_links|_terms|_term_taxonomy|_term_relationships|_usermeta|_users) /i",$query) ) $dbc_cachable = true;
...
// cache start
if ($dbc_cachable && !($dbc_cached = $dbc->load($dbc_queryid))) {
