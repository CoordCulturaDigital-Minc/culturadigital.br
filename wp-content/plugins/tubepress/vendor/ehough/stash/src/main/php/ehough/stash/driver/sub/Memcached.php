 ...
 */
class ehough_stash_driver_sub_Memcached
{
    /**
     * @var Memcached
     */
            ...
            switch ($name) {
                case 'HASH':
                    ...
                    if (!defined('Memcached::HASH_' . $value)) {
                        throw new ehough_stash_exception_RuntimeException('Memcached option ' . $name . ' requires valid memcache hash option value');
                    }
                ...
                case 'DISTRIBUTION':
                    ...
                    if (!defined('Memcached::DISTRIBUTION_' . $value)) {
                        throw new ehough_stash_exception_RuntimeException('Memcached option ' . $name . ' requires valid memcache distribution option value');
                    }
                ...
                case 'SERIALIZER':
                    ...
                    if (!defined('Memcached::SERIALIZER_' . $value)) {
                        throw new ehough_stash_exception_RuntimeException('Memcached option ' . $name . ' requires valid memcache serializer option value');
                    }
                ...
                case 'SERVER_FAILURE_LIMIT':
                    if (!is_numeric($value)) {
                        throw new ehough_stash_exception_RuntimeException('Memcached option ' . $name . ' requires numeric value');
                    }
                    break;

                case 'PREFIX_KEY':
                    if (!is_string($value)) {
                        throw new ehough_stash_exception_RuntimeException('Memcached option ' . $name . ' requires string value');
                    }
                    break;
                ...
                case 'CACHE_LOOKUPS':
                    if (!is_bool($value)) {
                        throw new ehough_stash_exception_RuntimeException('Memcached option ' . $name . ' requires boolean value');
                    }
                    break;
            }
