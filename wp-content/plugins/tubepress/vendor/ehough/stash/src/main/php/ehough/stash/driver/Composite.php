...
class ehough_stash_driver_Composite implements ehough_stash_driver_DriverInterface
{
    ...
    public function __construct(array $options = array())
    {
        if (!isset($options['drivers']) || !is_array($options['drivers']) || count($options['drivers']) < 1) {
            throw new ehough_stash_exception_RuntimeException('One or more secondary drivers are required.');
        }
        ...
        if (count($this->drivers) < 1) {
            throw new ehough_stash_exception_RuntimeException('None of the secondary drivers can be enabled.');
        }
    }
